...
| FR-HITL-06 | Immutable decision log | `CanonicalField`'s validator enforced at construction, at assignment in both directions, and on every update: `model_copy(update=...)` now raises rather than skipping validation, and `evolve(...)` revalidates (`test_resolution_invariant_survives_assignment`, `test_a_resolved_field_cannot_have_its_resolution_cleared`, `test_model_copy_update_is_refused_on_a_canonical_field`, `test_evolve_reruns_validation_and_still_forbids_the_state`). `ConflictQueueEntry` frozen (`test_a_recorded_resolution_cannot_be_replaced`); `Resolution`'s own fields frozen and tested (`test_resolution_fields_are_frozen`). **Two routes remain open:** the freeze is shallow, so `ConflictQueueEntry.candidates` is mutable in place; and `CanonicalField.resolution` can still be *replaced* by plain assignment — the validator checks status/resolution consistency, not that a decision is written once. Persisted, tamper-evident storage is NFR-02, still `declared` | partial |
