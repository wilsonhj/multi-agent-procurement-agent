...
> found ten defects by running the code. All ten are fixed and none of them changed what a
> requirement is covered by; the suite went from 481 passing to 523 by adding regression tests
> for behaviour that was already claimed or newly decided. **Two rows gained coverage without
> changing their status word:**
>
> - **FR-HITL-06** stays `partial`, and its citation is still accurate. What was missing — the
>   *reducer* discarded a recorded `Resolution`, so an idempotent re-run returned a resolved field
>   to `OPEN` (A-51) — is closed by [D-16](../specs/001-procurement-agent/clarifications.md): a
>   reviewer's decision is a `human:` claim carrying its `Resolution`, and
>   `test_an_idempotent_rerun_keeps_the_decision` pins the re-run. It stays `partial` because
>   persisted, tamper-evident storage is NFR-02's and the `claim` table has no resolution column
>   yet.
> - **FR-HITL-01** and **FR-WEB-04** now hold for the contract's `list[str]` fields too:
>   [D-17](../specs/001-procurement-agent/clarifications.md) adds `ToleranceRule.SET_EQUAL` and a
>   row per list key, so a reordered certification list is no longer a `CRITICAL` conflict
>   (A-53), pinned by `test_every_list_field_in_the_contract_has_a_set_rule`.
>
...
`PROCUREMENT_TEST_DSN`, so it **skips silently in a default local `pytest`** — the 26 skips in a
...
| FR-HITL-06 | Immutable decision log | **The first invariant is structural since D-18**: `CanonicalField` stores the unresolved state and `resolution`, and `conflict_status` is computed as RESOLVED exactly when a resolution is present, so RESOLVED-with-no-`Resolution` has no representation on any route — constructor, `evolve`, `model_construct`, pickle, `copy.copy`, `copy.deepcopy`, both `model_copy` forms, and a raw `__dict__` write (`test_the_forbidden_state_cannot_be_asked_for`, `test_no_copy_or_boundary_route_yields_the_forbidden_state`, `test_a_dict_write_cannot_fabricate_a_resolved_field`, `test_the_wire_shape_is_unchanged`, `tests/test_resolution_immutability.py`). **The second invariant is still a guard**: overwriting or clearing a recorded `Resolution` — which every validator passes, because the resulting state is legal — is refused by `__setattr__` and by `evolve`, and `model_copy(update=...)` is refused outright (`test_assignment_cannot_replace_a_recorded_resolution`, `test_evolve_cannot_replace_a_recorded_resolution`, `test_a_resolved_field_cannot_be_moved_off_resolved`); `ConflictQueueEntry` is frozen and `Resolution`'s own fields are frozen (`test_resolution_fields_are_frozen`). What a raw `__dict__` write can still do is *erase* a resolution, recorded as a fact in `test_a_dict_write_can_still_erase_a_decision_and_that_is_recorded`; it can no longer fabricate one. The reducer preserves a decision through a projection since [D-16](../specs/001-procurement-agent/clarifications.md) (`test_an_idempotent_rerun_keeps_the_decision`). The shallow-freeze gap is unchanged: `ConflictQueueEntry.candidates` is a list and is mutable in place. Persisted, tamper-evident storage is NFR-02; `sql/` implements the append-only half (no UPDATE/DELETE grant on `resolution`, plus row-level and statement-level tripwires), live-tested by `test_sql_behaviour.py::test_truncate_is_refused[public.resolution]` and `test_the_row_level_tripwire_still_raises`, but the `claim` table has no resolution column yet | partial |
