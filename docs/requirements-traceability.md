...
| AC-7 | Two generations from an unchanged store are byte-identical in **(a)** the canonical projection and **(b)** the rendered `.xlsx` | Scored per layer since the [A-46](../specs/001-procurement-agent/analysis.md) amendment. **(a)** is the criterion and is gated on C6 and G.1: the projection is not built, so nothing covers it. **(b)** is the renderer-regression check; `tests/test_workbook_determinism.py` covers `normalize_archive` at archive level, but `write_workbook` raises `NotImplementedError`, so no complete workbook is regenerated and the desktop Excel/LibreOffice gate (task G.6) is unrun. The pre-amendment version of this row scored the criterion against `normalize_archive` alone, which credited layer (b)'s partial coverage to a criterion that is now layer (a) | partial |
