...
UNIMPLEMENTED_REVIEW_ROUTING: dict[str, str] = {
    "flags_for has no tier notion": (
        ...
        "it. **This is live, not pending.** An earlier version of this note said "
        "'nothing is broken today because `write_workbook` raises "
        "NotImplementedError and `flags_for` has no live caller', and named the "
        "hazard as the next author wiring it up. It was wired up: "
        "`services/output/projection.py` imports `flags_for` and calls it in "
        "`_field_row`, which `project_store` reaches for every stored value. So "
        "the empty flag list is already inside the bytes `projection_digest` "
        'hashes - the C6 artifact of record carries `"flags": []` for a field '
        "the gate says no score can accept, and the xlsx writer's absence no "
        "longer bounds it. "
