...
|---|---|---|---|
| `ingest` | one document | fan-out | **The dominant win.** NFR-06 says hundreds of documents; parse and OCR are the expensive steps, averaging 3.1 s/page on CPU with a 16 s p95 tail. `content_hash` is the dedup key (NFR-05), and it is now a `UNIQUE` constraint: `sql/02_document.sql` refuses the duplicate rather than trusting the application to check first, live-tested by `test_a_duplicate_content_hash_is_refused`, so the retry path is `INSERT ... ON CONFLICT (content_hash) DO NOTHING`. NFR-05 and AC-5 are both `partial` — the constraint holds, the ingest caller that would exercise it does not exist. **This cell read "an unconstrained field today ... NFR-05 is `declared` and AC-5 `open`" until 2026-09-02**, and `sql/02_document.sql:41-50` quotes that exact sentence as the gap it closes, so the file implementing the fix cited the document still describing the defect. Process pool — CPU-bound, so threads buy nothing. |
