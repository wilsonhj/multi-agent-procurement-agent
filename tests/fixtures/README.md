...
ways — it validates against its model, it round-trips byte-for-byte, and it still
produces the behaviour it was written to encode. The third check is the one that
matters: schema-valid JSON encoding the wrong worked example is a failure the first
two cannot see.
...
- **Wire the directory into `FIXTURE_KINDS`** in `test_fixtures.py`. A fixture in an
  unmapped directory fails `test_every_fixture_lives_in_a_known_directory` rather
  than sitting there validated by nothing while looking covered.
...
There is no generator script on purpose: the round-trip test means a fixture can be
regenerated by loading it, revalidating, and re-dumping with
`json.dumps(..., indent=2, sort_keys=True)`. If a schema change makes a fixture fail
to round-trip, regenerate it **and re-check the behavioural assertions** — the point
of the fixture is what it means, not that it parses.
